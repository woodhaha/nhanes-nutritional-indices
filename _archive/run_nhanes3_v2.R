# run_nhanes3_v2.R — NHANES III with CORRECT column positions
# Rscript run_nhanes3_v2.R

library(dplyr); library(survival); library(broom)

DATA_DIR <- "D:/Researching/NHANES_aged_GI_tumor_nutrition/data/nhanes3"
GI_DATA_DIR <- "D:/Researching/data/gi_analysis"
GI_RESULTS_DIR <- "D:/Researching/results/gi_analysis"
dir.create(GI_RESULTS_DIR, recursive=TRUE, showWarnings=FALSE)

# ── Read only needed columns from .dat file ──────────────────────────────────
# Positions verified from adult.sas/lab.sas INPUT sections
read_columns <- function(file, col_spec) {
  # col_spec = list of list(name, start, end)
  con <- file(file, "r", blocking=TRUE)
  on.exit(close(con))

  # Read all lines
  lines <- readLines(con, warn=FALSE)
  n <- length(lines)

  # Initialize result
  result <- list()
  for (spec in col_spec) {
    result[[spec$name]] <- vector("character", n)
  }

  # Extract each column from each line
  pb <- floor(seq(1, n, length.out=11))
  for (i in seq_len(n)) {
    line <- lines[i]
    for (spec in col_spec) {
      result[[spec$name]][i] <- substr(line, spec$start, spec$end)
    }
  }

  # Convert to data.frame
  df <- as.data.frame(result, stringsAsFactors=FALSE)
  # Convert numeric columns
  for (nm in names(df)) {
    df[[nm]] <- suppressWarnings(as.numeric(trimws(df[[nm]])))
  }
  df
}

# ── Column specifications ────────────────────────────────────────────────────
# Adult file (LRECL=3348)
adult_cols <- list(
  list(name="SEQN", start=1, end=5),
  list(name="HSSEX", start=15, end=15),
  list(name="HSAGEIR", start=18, end=19),
  list(name="DMAETHNR", start=12, end=12),   # ethnicity
  list(name="DMARACER", start=13, end=13),   # race
  list(name="DMPPIR", start=36, end=41),
  list(name="WTPFEX6", start=61, end=69),
  list(name="SDPPSU6", start=43, end=43),
  list(name="SDPSTRA6", start=44, end=45),
  list(name="HFHEDUCR", start=1423, end=1424),
  list(name="HFF1", start=1356, end=1356),
  list(name="HFF20R", start=1411, end=1412),
  list(name="HFF19R", start=1409, end=1410),
  list(name="HAC1N", start=1478, end=1478),   # skin cancer
  list(name="HAC1O", start=1479, end=1479),   # other cancer
  list(name="HAC3OS", start=1527, end=1528)   # cancer site code
)

# Lab file (LRECL=1977)
lab_cols <- list(
  list(name="SEQN", start=1, end=5),
  list(name="AMP", start=1846, end=1848),       # albumin g/dL
  list(name="WCP", start=1273, end=1277),       # WBC count
  list(name="LMPPCNT", start=1283, end=1287),   # lymphocyte %
  list(name="LMP", start=1298, end=1302),       # lymphocyte #
  list(name="TCP", start=1598, end=1600)        # total cholesterol
)

# Exam file (verified from exam.sas INPUT: BMPBMI 1524-1527)
exam_cols <- list(
  list(name="SEQN", start=1, end=5),
  list(name="BMPBMI", start=1524, end=1527),
  list(name="BMPWT", start=1508, end=1513),
  list(name="BMPHT", start=1528, end=1532)
)

cat("=== NHANES III Data Extraction ===\n")

cat("Reading adult.dat...\n")
adult <- read_columns(file.path(DATA_DIR, "adult.dat"), adult_cols)
cat(sprintf("  %d rows\n", nrow(adult)))

cat("Reading lab.dat...\n")
lab <- read_columns(file.path(DATA_DIR, "lab.dat"), lab_cols)
cat(sprintf("  %d rows\n", nrow(lab)))

cat("Reading exam.dat...\n")
exam <- read_columns(file.path(DATA_DIR, "exam.dat"), exam_cols)
cat(sprintf("  %d rows\n", nrow(exam)))

cat("\n=== Verify values ===\n")
cat("Adult: SEQN head:", head(adult$SEQN, 3), "\n")
cat("Adult: age range:", range(adult$HSAGEIR, na.rm=TRUE), "\n")
cat("Adult: HAC3OS vals:", sort(unique(adult$HAC3OS[!is.na(adult$HAC3OS)])), "\n")
cat("Lab: AMP head:", head(lab$AMP, 5), "\n")
cat("Lab: TCP head:", head(lab$TCP, 5), "\n")
cat("Lab: LMP head:", head(lab$LMP, 5), "\n")
cat("Exam: BMPBMI head:", head(exam$BMPBMI, 5), "\n")

# ── Merge ────────────────────────────────────────────────────────────────────
df <- merge(adult, lab, by="SEQN", all.x=TRUE)
df <- merge(df, exam, by="SEQN", all.x=TRUE)

# Age filter
df <- df %>% filter(HSAGEIR >= 60)
cat(sprintf("\nAge >= 60: %d\n", nrow(df)))

# Drop implausible values (NHANES III uses 88888/99999 for unknown)
df <- df %>% filter(
  AMP >= 1.0 & AMP <= 7.0,              # albumin g/dL
  LMP >= 0.1 & LMP <= 20,               # lymphocyte ×10³/µL
  TCP >= 50 & TCP <= 500,               # total cholesterol mg/dL
  BMPBMI >= 10 & BMPBMI <= 80           # BMI
)
cat(sprintf("After plausibility filter: %d\n", nrow(df)))

# Cancer classification
GI_CODES <- c(6, 12, 13, 14, 15, 25)  # colon, stomach, esophagus, pancreas, liver, gallbladder
df <- df %>% mutate(
  any_cancer = ifelse((!is.na(HAC1N) & HAC1N==1) | (!is.na(HAC1O) & HAC1O==1), 1L, 0L),
  gi_tumor = ifelse(!is.na(HAC3OS) & HAC3OS %in% GI_CODES, 1L, 0L),
  gi_site = case_when(
    HAC3OS == 6 ~ "Colon", HAC3OS == 12 ~ "Stomach",
    HAC3OS == 13 ~ "Esophagus", HAC3OS == 14 ~ "Pancreas",
    HAC3OS == 15 ~ "Liver", HAC3OS == 25 ~ "Gallbladder",
    TRUE ~ NA_character_),
  age = HSAGEIR,
  sex = ifelse(HSSEX==1, "Male", "Female"),
  race_eth = case_when(
    DMAETHNR %in% c(1,2) ~ "Hispanic",
    DMARACER == 1 ~ "Non-Hispanic White",
    DMARACER == 2 ~ "Non-Hispanic Black",
    TRUE ~ "Other"),
  edu_binary = ifelse(HFHEDUCR >= 4, 1, 0),
  income_pir = DMPPIR,
  albumin_gdl = AMP,
  lymph_abs = LMP * 1000,  # LMP is ×10^3/µL → cells/µL
  tchol_mgdl = TCP,
  bmi = BMPBMI,
  # PNI = 10 × albumin(g/dL) + 0.005 × lymph(µL)
  PNI = 10*AMP + 0.005*(LMP*1000),
  CONUT = case_when(AMP>=3.5~0, AMP>=3.0~2, AMP>=2.5~4, TRUE~6) +
          case_when(LMP*1000>=1600~0, LMP*1000>=1200~1, LMP*1000>=800~2, TRUE~3) +
          case_when(TCP>=180~0, TCP>=140~1, TCP>=100~2, TRUE~3),
  GNRI = 14.89*AMP + 41.7*(bmi/22),
  cycle = "NHANES_III_1988-1994"
)

cat(sprintf("GI tumors: %d\n", sum(df$gi_tumor, na.rm=TRUE)))
print(table(df$gi_site[df$gi_tumor==1], useNA="ifany"))

# Complete nutrition
df <- df %>% filter(!is.na(PNI), !is.na(CONUT), !is.na(GNRI), !is.na(bmi))
cat(sprintf("Complete nutrition: %d (GI: %d)\n", nrow(df), sum(df$gi_tumor, na.rm=TRUE)))

cat("PNI: mean=", round(mean(df$PNI, na.rm=TRUE),1), "sd=", round(sd(df$PNI, na.rm=TRUE),1), "\n")
cat("albumin: mean=", round(mean(df$albumin_gdl, na.rm=TRUE),2), "\n")
cat("lymph_abs: mean=", round(mean(df$lymph_abs, na.rm=TRUE),0), "\n")
cat("tchol: mean=", round(mean(df$tchol_mgdl, na.rm=TRUE),1), "\n")

# ── Mortality ────────────────────────────────────────────────────────────────
mort <- readRDS(file.path(GI_DATA_DIR, "mort_nhanes3.rds"))
df <- merge(df, mort[, c("SEQN","eligstat","mortstat","ucod_leading","permth_int")],
            by="SEQN", all.x=TRUE)
df <- df[df$eligstat==1 & !is.na(df$mortstat), , drop=FALSE]
df$surv_years <- df$permth_int/12
df$death <- df$mortstat
df$gi_cancer_death <- ifelse(df$death==1 & df$ucod_leading==2 & df$gi_tumor==1, 1L, 0L)

cat(sprintf("Follow-up: %d, deaths: %d, GI cancer deaths: %d\n",
            nrow(df), sum(df$death), sum(df$gi_cancer_death, na.rm=TRUE)))

saveRDS(df, file.path(GI_DATA_DIR, "nhanes3_gi_merged.rds"))

# ── Combine ──────────────────────────────────────────────────────────────────
cat("\n=== Combine with NHANES 2005-2016 ===\n")
df5 <- readRDS(file.path(GI_DATA_DIR, "nhanes_gi_nutrition_raw.rds"))
mort5 <- readRDS(file.path(GI_DATA_DIR, "mort_2019.rds"))
df5 <- merge(df5, mort5[,c("SEQN","eligstat","mortstat","ucod_leading","permth_int")],
             by="SEQN", all.x=TRUE)
df5 <- df5[df5$eligstat==1 & !is.na(df5$mortstat),]
df5$surv_years <- df5$permth_int/12
df5$death <- df5$mortstat
df5$gi_cancer_death <- ifelse(df5$death==1 & df5$ucod_leading==2 & df5$gi_tumor==1, 1L, 0L)
df5$cycle <- paste0("NHANES_", df5$cycle)

common <- intersect(names(df5), names(df))
df_all <- bind_rows(df5[, common], df[, common])

cat(sprintf("COMBINED: N=%d, GI tumors=%d\n", nrow(df_all), sum(df_all$gi_tumor, na.rm=TRUE)))
cat(sprintf("  NHANES III: %d\n", sum(df$gi_tumor, na.rm=TRUE)))
cat(sprintf("  2005-2016:  %d\n", sum(df5$gi_tumor, na.rm=TRUE)))

# ── Survival ─────────────────────────────────────────────────────────────────
cat("\n── Survival Analysis ──\n")
df_gi <- df_all %>% filter(gi_tumor==1, surv_years>0) %>%
  mutate(PNI_s=as.numeric(scale(PNI)),
         CONUT_s=as.numeric(scale(-CONUT)),
         GNRI_s=as.numeric(scale(GNRI)),
         sex_b=ifelse(sex=="Female",1L,0L),
         race_eth_b=ifelse(race_eth=="Non-Hispanic White",1L,0L))

n_gi <- nrow(df_gi); n_ev <- sum(df_gi$death)
cat(sprintf("  N=%d, events=%d\n", n_gi, n_ev))

if (n_ev >= 10) {
  cox_res <- data.frame()
  for (exp in c("PNI_s","CONUT_s","GNRI_s")) {
    for (adj in c("Crude","Adjusted")) {
      covs <- if(adj=="Crude") "1" else "age + sex_b + race_eth_b"
      f <- as.formula(paste("Surv(surv_years,death) ~", exp, "+", covs))
      fit <- coxph(f, data=df_gi)
      hr <- tidy(fit, conf.int=TRUE) %>% filter(term==exp)
      cox_res <- rbind(cox_res, data.frame(
        Nutrition=gsub("_s","",exp), Adjustment=adj,
        HR=exp(hr$estimate), Lower=exp(hr$conf.low),
        Upper=exp(hr$conf.high), P=hr$p.value, N=n_gi, Events=n_ev))
    }
  }
  cox_res$HR_CI <- sprintf("%.3f (%.3f-%.3f)", cox_res$HR, cox_res$Lower, cox_res$Upper)
  cat("\nAll-cause Cox:\n")
  print(cox_res[, c("Nutrition","Adjustment","HR_CI","P")])
  write.csv(cox_res, file.path(GI_RESULTS_DIR,"cox_allcause_combined.csv"), row.names=FALSE)
}

cat("\nDone.\n")
